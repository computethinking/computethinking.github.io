---
hide:
  - navigation
  - toc
  - footer
---
# **Computational Thinking for Business: BMIS 2402**


<div class="grid cards" markdown>

-   :material-text-long:{ .lg .middle } __Course Information__

    ---

    [:octicons-arrow-right-24: Course Description](syllabus/course-description.md)

    [:octicons-arrow-right-24: Schedule](syllabus/schedule.md)

    [:octicons-arrow-right-24: Course Material](syllabus/course-materials.md)

-   :material-clock-fast:{ .lg .middle } __Instructor's Office & Classroom__

    ---


    [:octicons-arrow-right-24: Instructor Info Page via Canvas](https://canvas.pitt.edu/courses/324345/pages/instructor-info-2)

    [:octicons-arrow-right-24: Virtual Office via Zoom](https://pitt.zoom.us/my/midhubalan)

    [:octicons-arrow-right-24: Email](mailto:midhu.balan@pitt.edu)


-   :material-book-check-outline:{ .lg .middle } __Grading & Evaluation__

    ---


    [:octicons-arrow-right-24: Grading Information](syllabus/grading.md)

    [:octicons-arrow-right-24: Canvas Assignment Page](https://canvas.pitt.edu/courses/324345/assignments)

-   :octicons-beaker-24:{ .lg .middle } __Tools__

    ---

    [:octicons-arrow-right-24: Google AI Studio](https://aistudio.google.com/)

    [:octicons-arrow-right-24: Posit RStudio Desktop](https://posit.co/download/rstudio-desktop/)

-   :material-gold:{ .lg .middle } __Resources and References__

    ---

    [:octicons-arrow-right-24: O'Reilly](https://learning.oreilly.com/library/view/temporary-access/)

    [:octicons-arrow-right-24: LinkedIn Learning](https://linkedinlearning.pitt.edu/)

    [:octicons-arrow-right-24: R & RStudio Cheatsheets](https://posit.co/resources/cheatsheets/https://canvas.pitt.edu/courses/320955)



</div>